...
- 2026-09-21 사용자 정정: 대화 내 문서 선택은 탐색·미리보기·선택·해제·범위 적용 전용이다.
  아래의 대화 패널 inline 관리 계획은 철회한다. 업로드·새 폴더·새 버전·이동 등 관리는 독립 파일함에서만 제공한다.
...
Platform 파일함과 도메인 파일함은 관리 기능을 제공한다. 대화 내 패널은 같은 탐색 컴포넌트를 읽기 전용으로 재사용해 검색할 문서만 선택한다. 탐색·미리보기·검색 선택은 각각 별도의 상태이며 파일을 여는 것만으로 검색 범위가 변하지 않는다.
