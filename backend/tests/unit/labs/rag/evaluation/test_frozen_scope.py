...
async def test_frozen_scope_preserves_exact_snapshot_ids_and_inactive_semantics() -> None:
    ...
    resolved = await FrozenResolvedScope(frozen).resolve(
        ...
        document_ids=None,
        document_processing_profile_id=None,
    )
...
@pytest.mark.asyncio
async def test_frozen_scope_rejects_selected_document_arguments() -> None:
    frozen = ResolvedSearchScope(
        workspace_ids=(uuid4(),),
        folder_ids=(),
        active_only=False,
    ...
    with pytest.raises(RuntimeError, match="does not accept document selection"):
        await FrozenResolvedScope(frozen).resolve(
            actor_id=uuid4(),
            workspace_ids=frozen.workspace_ids,
            folder_ids=frozen.folder_ids,
            indexing_profile_id=uuid4(),
