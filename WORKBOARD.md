...
격리된 `feature/workshop-foundation` 브랜치에서 구현 계획의 Task 6 영속 백그라운드 작업 기반을 테스트 우선으로 구현한다.
...
- 현재 영속 작업 상태, 멱등성 키와 워커 실행 기반을 구현할 차례다.

### 완료 기준

- 작업 요청이 영속 작업 ID와 상태를 반환한다.
- 동일 멱등성 키의 중복 요청이 작업을 중복 생성하지 않는다.
- 워커가 작업 상태를 대기·실행·성공 또는 실패로 일관되게 전이한다.

## 최근 완료 작업

최근 완료 작업은 가장 최신 항목부터 **최대 5개만 유지한다**.

1. 폴더 API, 권한 확인 문서 업로드, 불변 새 버전 등록과 문서 브라우저를 구현하고 검증했다.
2. 회사·팀·개인·임시 지식 공간과 멤버십, 공간 구분 UI를 구현했다. 관련 커밋: `24fe1b4`
3. 소유자 Argon2 인증, JWT HttpOnly 쿠키 세션, 부트스트랩 CLI와 로그인 UI를 구현했다. 관련 커밋: `36f9f39`
4. 환경 설정, SQLAlchemy async 기반, Alembic과 공통 오류 계약을 구현하고 검증했다. 관련 커밋: `10673d4`
5. React·FastAPI 실행 골격과 PostgreSQL·Redis Compose 구성을 테스트 우선으로 구현했다. 관련 커밋: `a7a8161`

## 다음 작업

1. Task 6 영속 백그라운드 작업과 워커 기반을 구현한다.
2. 이후 계획의 Task 7부터 Task 9까지 순차 실행한다.
...
- 현재 저장소에는 Task 1부터 Task 5까지의 실행 가능한 애플리케이션 기반과 설계 문서가 있다.
