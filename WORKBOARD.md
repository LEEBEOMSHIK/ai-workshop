...
- 마지막 갱신일: 2026-09-02
- 현재 단계: 공개 AI Lab과 다중 도메인 RAG 서비스 상위 설계 사용자 검토
- 전체 상태: 공개 캐릭터 탐색, 다중 도메인, 불변 공개 릴리스, 대화형 LLM 답변, 로그인 첨부·피드백과 수동 파인튜닝 경계를 설계 문서로 작성했으며 구현은 시작하지 않았다.

## 현재 작업

### 목표

작성된 상위 설계를 사용자가 검토하고 승인하면 1단계 `공개·작업소·관리자 경계 복원`만 대상으로 상세 구현 계획을 수립한다. 사용자 검토 전에는 라우트와 권한 동작을 변경하지 않는다.
...
1. `docs/superpowers/specs/2026-09-02-public-ai-lab-rag-service-design.md` 사용자 검토와 승인
2. 승인 뒤 1단계 `공개·작업소·관리자 경계 복원` 상세 구현 계획 작성
3. 형식별 Parser Policy Version과 Saved RAG Configuration 연결, 재파싱·재청킹·재색인 수명주기 설계
4. Parser Policy에 포함되는 DOCX 구조 파서와 원문 뷰어 계약 설계

## 결정이 필요한 항목

- 작성된 상위 설계 문서의 사용자 검토와 구현 계획 전환 승인이 필요하다.
