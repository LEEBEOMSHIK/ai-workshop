...
- 저장소는 Next.js React 프론트엔드와 FastAPI 백엔드를 포함하는 모듈형 모놀리스로 시작한다.
...
- 완성된 대화형 RAG에는 근거 제한 LLM 답변과 인용 검증이 필요하다.
- 리랭커는 선택 사항이며 미설정 상태를 오류로 취급하지 않는다.
- 구성한 선택 모델이나 필수 모델이 실패하면 다른 모델로 조용히 전환하지 않는다.
...
- 사용자 실행 검증을 위해 별도 환경·DB·계정을 임의로 만들지 않는다. 기존 환경의 계정·자료를 보존해 검증한다.
- 별도 환경에서의 성공을 본래 환경 완료로 보고하지 않는다. 필요한 변경의 본래 환경 반영·검증 전에는 별도 환경을 삭제하지 않는다.
...
- 사용자 보고·발견한 결함은 [문제 이력 규칙](docs/issues/README.md)에 따라 누적 기록하고 수정·검증·재발 시 갱신한다.
