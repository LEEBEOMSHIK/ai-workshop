...
it("submits server search with filters and shows total separately from filtered count", () => {
    render(<IssueHistoryPage list={{ items: [], total: 0, status_counts: { open: 5, implemented: 1, verified: 2 } }} categories={[root, category]} selected={null} filters={{ q: "abc" }}/>);
...
it("preserves failed input and request id when retrying a mutation", async () => {
    ...
    render(<IssueManagement categories={[root, category]} issue={issue} onSaved={vi.fn()}/>);
...
it("keeps the original revision when refreshed data arrives during editing", async () => {
    ...
    const user = userEvent.setup();
    const { rerender } = render(<IssueManagement categories={[root, category]} issue={issue} onSaved={vi.fn()}/>);
    await user.click(screen.getByText("선택한 문제 편집"));
    const title = screen.getAllByLabelText("문제 제목")[1];
    await user.clear(title);
    await user.type(title, "작성 중");
    rerender(<IssueManagement categories={[root, category]} issue={{ ...issue, revision: 2, title: "다른 변경" }} onSaved={vi.fn()}/>);
...
it("advances the expected revision after successful saves", async () => {
    ...
    const form = title.closest("form")!;
    await user.click(within(form).getByRole("button", { name: "저장" }));
    rerender(<IssueManagement categories={[root, category]} issue={{ ...issue, revision: 2 }} onSaved={vi.fn()}/>);
...
it("registers a child category with its selected parent",async()=>{
 ...
 await user.click(screen.getByText("카테고리 관리"));await user.click(screen.getByText("카테고리 등록"));
 await user.type(screen.getByLabelText("고유 코드"),"child");await user.type(screen.getByLabelText("카테고리 이름"),"새 하위 분류");
 await user.selectOptions(screen.getByLabelText("상위 분류 연결"),"rag");
 const form=screen.getByLabelText("고유 코드").closest("form")!;
 await user.click(within(form).getByRole("button",{name:"저장"}));
