...
격리된 `feature/workshop-foundation` 브랜치에서 구현 계획의 Task 8 OpenAPI 계약과 생성 TypeScript 타입을 테스트 우선으로 구현한다.
...
- OpenAPI에 공개 경로, 공통 오류, 보안 스키마와 고유 operation ID를 명시한다.
- 결정적으로 내보낸 OpenAPI로 TypeScript 계약 타입을 생성하고 변경 여부를 검사한다.
- 공통 fetch client와 각 기능 API가 생성 타입, 쿠키, multipart, correlation ID 계약을 사용한다.

## 최근 완료 작업

최근 완료 작업은 가장 최신 항목부터 **최대 5개만 유지한다**.

1. 불변 RAG 모델·프로파일 레지스트리, 종류별 검증, YAML 등록 API와 Model Lab UI를 구현하고 검증했다.
2. 영속 작업 상태, 멱등 생성, Celery worker, 객체 검증, 작업 조회 API와 수동 상태 UI를 구현하고 검증했다.
3. 폴더 API, 권한 확인 문서 업로드, 불변 새 버전 등록과 문서 브라우저를 구현했다. 관련 커밋: `b1411f8`
4. 회사·팀·개인·임시 지식 공간과 멤버십, 공간 구분 UI를 구현했다. 관련 커밋: `24fe1b4`
5. 소유자 Argon2 인증, JWT HttpOnly 쿠키 세션, 부트스트랩 CLI와 로그인 UI를 구현했다. 관련 커밋: `36f9f39`

## 다음 작업

1. Task 8 OpenAPI 계약과 생성 TypeScript 타입을 구현한다.
2. 이후 계획의 Task 9를 실행한다.
...
- 현재 저장소에는 Task 1부터 Task 7까지의 실행 가능한 애플리케이션 기반과 설계 문서가 있다.
