...
pytestmark = [
    ...
    pytest.mark.skipif(
        ...
        reason="Set AI_WORKSHOP_MODEL_SMOKE=1 to load the pinned local BGE-M3 model.",
    ),
]
...
def test_pinned_bge_m3_emits_normalized_dense_1024_dimension_vector() -> None:
    definition = next(
        model for model in load_model_catalog(CATALOG_ROOT) if model.id == BGE_ID
    )
    ...
    vector = embedding.encode_documents(["Public synthetic local BGE-M3 smoke evidence."])[0]
