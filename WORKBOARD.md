...
- 마지막 갱신일: 2026-09-02
- 현재 단계: 프로젝트 개발 에이전트 조직 설계 작성 완료, 사용자 명세 검토 대기
- 전체 상태: 도구 독립 역할 계약, 메인 오케스트레이터의 온디맨드 선택, 계층형 공통·RAG 역할, 전체 소스 하드코딩 방지, 임시 기록 자동 정리와 독립 검증 게이트를 설계했다.
...
1. `docs/superpowers/specs/2026-09-02-project-development-agent-organization-design.md`를 사용자가 검토한다.
2. 승인된 명세를 바탕으로 상세 구현 계획을 작성한다.
3. 역할 계약·오케스트레이션 어댑터·임시 기록 수명주기·검증 도구를 구현한다.
4. 프로젝트 에이전트 조직 도입 뒤 DOCX 구조 파서·원문 뷰어 계약 설계로 복귀한다.

## 결정이 필요한 항목

- 프로젝트 개발 에이전트 조직 명세에 대한 사용자 최종 검토가 필요하다.

## 차단 요소

- 구현 계획 작성은 프로젝트 개발 에이전트 조직 명세의 사용자 검토가 끝날 때까지 진행하지 않는다.
