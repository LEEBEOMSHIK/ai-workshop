...
_DATABASE_NAME = re.compile(r"ai_workshop_learning_[0-9a-f]{32}")


@dataclass(frozen=True, slots=True)
class IsolatedLearningDatabase:
    config: Config
    database_url: str
    name: str
...
def _require_exact_database_name(database: str) -> None:
    if _DATABASE_NAME.fullmatch(database) is None:
        raise AssertionError("refusing to operate on a non-learning test database")
...
@contextmanager
def isolated_learning_database(
    monkeypatch: pytest.MonkeyPatch,
) -> Iterator[IsolatedLearningDatabase]:
    settings = get_settings()
    database = f"ai_workshop_learning_{uuid4().hex}"
    ...
    try:
        ...
        print(f"learning_test_database_dropped={database}")
