"""Seed a separate immutable scanned PDF and DOCX OCR profile."""
...
revision: str = "0021_pdf_ocr_profile"
down_revision: str | Sequence[str] | None = "0020_pp_structure_profile"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

PROFILE_ID = UUID("00000000-0000-0000-0000-000000000210")
MODEL_BINDINGS = (
...
def _seed_profile() -> None:
        ...
        connection.execute(
            ...
            WHERE id = :profile_id
               OR (kind = 'document_processing'
                   AND name = 'pp-structure-v3-pdf-docx' AND version = 1)
            """
            ),
    ...
    if rows:
        ...
        if (
            ...
            raise RuntimeError("PP-StructureV3 PDF profile identity conflicts.")
        return
    connection.execute(
        sa.text(
            """
            INSERT INTO rag_profiles (
                ...
                :profile_id, 'document_processing', 'pp-structure-v3-pdf-docx', 1,
...
def _seed_binding(index: int, model_id: UUID, role: str) -> None:
    connection = op.get_bind()
    binding_id = UUID(f"00000000-0000-0000-0000-{350 + index:012d}")
    rows = (
        connection.execute(
            sa.text(
                """
    ...
    if rows:
        ...
        if (
            ...
            raise RuntimeError(f"PP-StructureV3 PDF binding conflicts: {role}.")
        return
    connection.execute(
        sa.text(
            """
