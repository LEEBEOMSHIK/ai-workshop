...
export function ConfigurationBuilder({
  ...
  const embeddingModel = embeddingFor(indexingProfile, models);
    ...
    <section className="configuration-builder" aria-labelledby="configuration-builder-title">
      <div className="section-heading-row">
        <div>
          ...
          <h2 id="configuration-builder-title">새 조합 만들기</h2>
        </div>
      ...
      <form onSubmit={handleSubmit}>
            ...
            >
              {indexingProfiles.map((profile) => (
                <option key={profile.id} value={profile.id}>
                  {profile.name} v{profile.version} · {profile.id}
                </option>
              ))}
            </select>
          </label>
          <ReadOnlyComponent label="Parser" value={profileComponent(indexingProfile, "parser")} />
          <ReadOnlyComponent label="Chunker" value={chunkerIdentity(indexingProfile)} />
          <ReadOnlyComponent label="Embedding" value={modelIdentity(embeddingModel)} />
          {requiresNewIndex ? (
        ...
        <fieldset disabled={saving}>
          ...
          <ReadOnlyComponent label="BM25" value={hasConfigKey(retrievalProfile, "bm25") ? "사용" : "사용 안 함"} />
          <ReadOnlyComponent
            label="Dense"
            value={hasDense(retrievalProfile) ? modelIdentity(embeddingModel) : "사용 안 함"}
          />
          <ReadOnlyComponent label="RRF" value={profileComponent(retrievalProfile, "rrf")} />
        </fieldset>
...
function embeddingFor(
  ...
  const modelId = profile?.bindings.find((binding) => binding.role === "embedding")?.model_id;
  return models.find((model) => model.id === modelId && model.kind === "embedding");
}
...
function profileComponent(profile: Profile | undefined, key: string): string {
  if (!profile || !(key in profile.config)) return "프로파일에 명시되지 않음";
  return formatJsonValue(profile.config[key]);
}

function chunkerIdentity(profile: Profile | undefined): string {
  if (!profile) return "프로파일에 명시되지 않음";
  if ("chunker" in profile.config) return formatJsonValue(profile.config.chunker);
  const target = profile.config.target_tokens;
  const overlap = profile.config.overlap_tokens;
  if (typeof target === "number" || typeof overlap === "number") {
    return `target ${String(target ?? "-")} · overlap ${String(overlap ?? "-")}`;
  }
  return "프로파일에 명시되지 않음";
}

function formatJsonValue(value: Profile["config"][string] | undefined): string {
  if (value === undefined) return "프로파일에 명시되지 않음";
