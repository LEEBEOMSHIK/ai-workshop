...
function CategoryParentSelect({categories, category}: {categories: Category[]; category?: Category}) {
    ...
    return <label>상위 분류 연결<select name="parent_id" defaultValue={category?.parent_id ?? ""}>
        <option value="">상위 카테고리로 등록</option>
        {!hasChildren && roots.map(item => <option key={item.id} value={item.id}>{item.name}{!item.is_active ? " (비활성)" : ""}</option>)}
    </select>{hasChildren && <small>하위 분류가 있어 상위 카테고리를 유지합니다.</small>}</label>;
}
