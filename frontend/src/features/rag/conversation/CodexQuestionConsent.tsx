...
export function CodexQuestionConsent({ classification, consented, disabled, onClassification, onConsent }: {
  ...
  return <div className="control-help">
    ...
    <label className="external-question-confirmation"><input type="checkbox" aria-label="이번 질문의 외부 처리를 확인했습니다" checked={consented} disabled={disabled} onChange={(event) => onConsent(event.target.checked)} />이번 질문의 외부 처리를 확인했습니다</label>
    <small>관리자가 저장한 전송 승인과 별개로, 매 질문 전에 외부 전송과 계정 사용량 발생에 동의하는 절차입니다.</small>
  </div>;
}
